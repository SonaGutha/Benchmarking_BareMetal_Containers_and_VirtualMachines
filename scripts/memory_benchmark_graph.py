...
def pivot_virtualization(grouped_data):
    ...
    return grouped_data.pivot(index='threads', columns='virtualizationType', values=['totaloperations', 'throughput','efficiency'])
...
def throughput(ax, data, label):
    ...
    data.plot(kind='bar', ax=ax,  label=label)
    ax.set_ylabel('Throughput(MiB/sec)')
    ax.legend(loc="upper right")
    ax.set_title(label)
...
def show_graph():
    ...
    graph.savefig(os.path.join(os.path.dirname(os.path.abspath(__file__)),'..','graphs','memory_benchmark.png'))
...
def main(path):
    ...
    # plots Throughput
    throughput(axes[2], pivot_data['throughput'], label='Throughput(MiB/Sec) Graph')

    set_subplot_labels(axes, pivot_data)

    # set graph title
    fig.suptitle('Memory Benchmark Results', y=1)

    show_graph()

main(os.path.join(os.path.dirname(os.path.abspath(__file__)),'..','input','memory_benchmark.txt'))
