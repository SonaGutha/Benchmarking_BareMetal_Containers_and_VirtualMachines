...
def pivot_virtualization(grouped_data):
    ...
    return grouped_data.pivot(index='threads', columns='virtualizationType', values=['efficiency', 'avgLatency', 'throughput'])
...
def throughput(ax, data, label):
    ...
    data.plot(kind='bar', ax=ax,  label=label)
    ax.set_ylabel('Measured Throughput(events/sec)')
    ax.legend(loc="upper right")
    ax.set_title(label)
...
def show_graph():
    ...
    graph.savefig(os.path.join(os.path.dirname(os.path.abspath(__file__)),'..','graphs','cpu_benchmark.png'))
...
def main(path):
    ...
    # plots Avg. Latency
    latency(axes[1], pivot_data['avgLatency'], label='Avg. Latency(ms) Graph')

    # plots Throughput
    throughput(axes[2], pivot_data['throughput'], label='Measured Throughput(events per second) Graph')

    set_subplot_labels(axes, pivot_data)

    # set graph title
    fig.suptitle('CPU Benchmark Results', y=1)

    show_graph()

main(os.path.join(os.path.dirname(os.path.abspath(__file__)),'..','input','cpu_benchmark.txt'))
